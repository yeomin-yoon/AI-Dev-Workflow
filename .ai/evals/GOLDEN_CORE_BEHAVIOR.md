...
- As part of the checkpoint and before staging, the run ledger receives exactly one appended accepted-Task line marked `closure: main_checkpoint`, committed with the change it describes and removed again if that checkpoint cannot commit, whose every field is derived from the Task, Build, Review, code-inspection, and checkpoint artifacts. A non-`main` sealed candidate instead receives one `lane_handoff` line from whichever role creates the Lane handoff commit, and an unusable Git records `accepted_no_git`; no delivery mode closes an accepted Task without a line. It is never mentioned in chat, never offered as a choice, and never gates the next Task; an unwritable ledger changes nothing about the route. No line is appended for a superseded, abandoned, or still-open Task, and no provider, model, token, timing, or file-count value is recorded.

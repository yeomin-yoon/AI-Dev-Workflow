...
- `variable-detail-request`: a short seed, detailed specification, referenced document, broad collaborative seed, or tacit/evaluative signal preserves all supplied intent; project-file context fills known gaps without re-asking; broad planning first fixes the current design altitude, bounded deliverable, deferred depth, and stop condition; `needed now` rejects premature detail/questions/writes; a tacit signal triggers one bounded evidence-based diagnosis before questions or Task creation, and only consequential missing user-owned intent produces questions (Golden Core Fixture 1)
...
- `task-quality-gate`: proposed slices resolve to READY, SPLIT, MERGE, or BLOCKED from one outcome, independent delivery, complete narrow writes, observable ACs, executable verification, ready dependencies, and handoff value; only READY reaches Builder, a narrow end-to-end/vertical outcome is preferred over a horizontal scaffold without standalone approved value, and the exact intent -> Architecture slice -> Task AC -> verification lineage contains no orphan, contradiction, or silently omitted finish behavior (Golden Core Fixture 6)
...
- `intent-anchored-bounded-diagnosis`: active delivery reads exact approved intent/ACs before causes or choices, filters repairs through approved ownership/responsibility/dependency boundaries, reuses still-applicable verified project evidence, tests precedent fit before invention, reaches official/domain guidance before a last-resort reversible bounded experiment, separates observed/inferred/confirmed evidence, batches one action-first manual check, makes structural authoring self-contained with whole-flow/first-use-term/finished-shape guidance, promotes only current blockers, prevents nested problem chains, and keeps unconfirmed hypotheses out of authoritative artifacts (Golden Core Fixture 23)
...
- `post-integration-lane-continuation`: another Task in the same approved Lane starts in a fresh Branch/worktree pinned to current main, performs targeted state/Knowledge validation, and never reuses divergent pre-integration history; after the last candidate, Feature convergence precedes any `synced/idle` disposition
...
- `artifact-authority-single-source`: Artifact Authority is the only fact-ownership/artifact-evolution/conflict-action authority; it separates reference-only user/team intent, living current views, flow-forward completed evidence, and live implementation without creating duplicate Specs or rewriting history (Golden Core Fixture 7)
...
- `terminal-no-knowledge-transition`: final PASS with sync none, no pending Review, and no active work reaches bounded Architect Feature convergence without an empty Knowledge handoff; any open/conflicting outcome keeps the Feature active even beside an approved deferral, only fully implemented/approved-excluded scope is called complete, user-approved deferral remains visibly incomplete, and historical Architecture without Delivery Slices is losslessly reconstructed or kept open rather than treated as completion evidence (Golden Core Fixture 4)
...
- a last Task PASS never substitutes for Feature intent coverage: at the natural boundary, current approved scope/slices reconcile to accepted exact candidates, live implementation, and decisive evidence; specified open work continues without a redundant Gate, conflicts return to their existing owner, historical missing slice maps never become completion evidence, and only user-approved deferred work may rest visibly incomplete
...
- broad collaborative planning stays at one named design altitude with a bounded deliverable, explicitly deferred deeper levels, and a stop condition; only details needed now may create questions or writes, concise assent binds only one displayed semantic outcome, and explicit confusion/surrender never supplies product authority
...
- every pre-Build, Build, Review, and Integration blocker type covered by the lifecycle has a defined repair/resume path, and terminal Task or Integration PASS without remaining Knowledge work reaches Architect Feature convergence before any `synced/idle` disposition

...
2. **Establish the smallest sufficient whole, then build through it.** User and Architect reason from the lowest altitude that contains the material decision—project, system, Feature, component/interaction, or Task/program shape—until purpose, ownership, critical flow, invariants, failure/exit behavior, verification, and deliberately deferred detail are coherent at that boundary. They then prove the structure with the smallest executable end-to-end backbone and deliver one approved vertical Task candidate at a time. A small change does not trigger a whole-project redesign, and a broad design does not continue into reversible detail merely because more can be specified.
3. **Input depth follows the user.** A request may be a short seed, a detailed specification, referenced documents, or a tacit/evaluative signal such as "something feels off." Preserve every explicit goal, rationale, environment, priority, constraint, acceptance condition, and requested deliverable; derive available context from authoritative project files; ask only for missing user-owned intent that materially changes scope, design, risk, or acceptance. For broad collaborative planning, orient one bounded pass by its current design altitude, deliverable, explicitly deferred deeper levels, and stop condition; ask or specify a detail only when it is needed now to finish that pass or unlock its decisive evidence. Evaluative or tacit seeds are valid problem signals, not failed requirements: before asking the user to diagnose the system, use targeted evidence to translate the signal into an observable symptom, bounded causal hypotheses with uncertainty, the most likely explanation, and the smallest discriminating probe or proposed improvement. Surface a relevant non-obvious possibility without silently adding it to scope, and turn a sufficiently resolved diagnosis into observable acceptance criteria before Build. Never require a form or re-ask a known fact. Durable product/requirements/specification sources are optional and, when used, are referenced by exact requirement section and revision rather than copied wholesale.
...
7. **Informed user judgment and continuous source truth.** AI makes reversible local choices and routine verified operations without interruption. A user Gate exists only when the user has a real choice between materially different user-owned outcomes, scope, public compatibility, irreversible/external effects, cost/risk, or mandatory human acceptance. Before that Gate, expose evidence, concrete observable consequences, recommendation, deferral/rejection consequence, uncertainty, and reconsideration conditions in language the user can use; an unexplained or effectively unavoidable approval is ceremony, not valid control. A concise assent is valid after one decision-ready semantic choice, while confusion or surrender is not informed approval and triggers reclassification, simplification, evidence, or deferral instead of silent transfer of product authority. The Workflow makes a skilled development process usable immediately without reducing AI assistance: from design through Build and Review it preserves one semantic thread from observable intent to responsibility, runtime/data flow, exact source symbols, invariants, and evidence. The required base stays visible for every user; definitions, examples, alternatives, and theory expand only when the current conversation needs them. This supports natural learning without quizzes, scores, forced replies, or a separate teaching path, but does not guarantee skill growth. Independent AI Review reduces review burden but never transfers code ownership or proves long-term maintainability.
8. **Engineering quality is intent, safety, and justified change—not pattern count.** Names, types, ownership, and APIs should expose real domain intent and protect invariants. Responsibilities follow reasons to change; extension points and performance work require actual pressure or evidence. Approved observable intent and responsibility, ownership, and dependency boundaries filter implementation candidates before convenience: a boundary-violating workaround does not become viable merely because it is easier to implement or test. Compare verification strength, simplicity, reversibility, and cost only among boundary-consistent candidates. Applicable sourced team/project rules outrank these generic heuristics; otherwise prefer simple project-consistent code over ceremonial SOLID, speculative polymorphism, or clever syntax.
...
10. **Model-agnostic behavioral floor without assumed parity.** Providers and model strengths may change while the same file contracts remain usable. Keep explicit only project-specific decisions, authority/safety boundaries, recurrent escaped failures, and judgment procedures whose variation can materially change the result; own each once and protect important behavior with positive and inverse evidence. Do not duplicate generic advice across roles merely because one model may already know it. Practical equivalence is an Eval conclusion, never a design claim.
...
`.ai/contracts/ARTIFACT_AUTHORITY.md` is the single authority for fact ownership, artifact evolution, and conflict actions. Load it only when the authoritative artifact/lifetime is unclear or sources disagree.
...
→ executable backbone + first vertical Task candidate
→ meaningful approval only when user-owned consequence differs
→ build candidate + continuous source-truth orientation
→ independent review
→ accepted change + risk-scaled verified Diff/source trace
...
- Before inventing a design or repair, apply `.ai/roles/ARCHITECT.md#decision-evidence-ladder`: reuse still-applicable verified project evidence, test the fit of relevant precedent and official/domain guidance, and use a reversible bounded experiment only for material uncertainty that those sources cannot resolve. Precedent is evidence rather than authority, and a trivial or already-determined choice creates no research or experiment ceremony.
...
- Architecture, Build, and Review: keep one intent-to-code thread. Architecture names the current responsibility/flow and existing or planned source boundary when evidence can locate it; Builder replaces plans with exact `path#symbol` truth as soon as implementation entry points are known and reports only material map changes; Review verifies the cumulative map against the exact candidate and presents the smallest connected source path that explains the result. Use conceptual/runtime order before file order; mechanical/non-code changes never gain a ceremonial pause.

Decision authority, delivery, and learning visibility are separate. A local/reversible technical choice does not become a user Gate, and a learning explanation never pauses implementation. The always-visible base is `observable result -> responsibility/flow -> exact current source anchor when known -> invariant/evidence limit -> next action`. A non-obvious choice that changes responsibility, class/interface shape, control flow, or reversibility adds its project-grounded reason, one meaningful alternative/tradeoff, and reconsider/revert condition without hiding that base. Do not infer a permanent novice/expert mode or reduce AI assistance: reuse already-understood terms in the current conversation, define an unfamiliar term at first use, and expand examples or theory only after a confusion signal, material risk, or request. The user never needs prerequisite external study merely to understand the current decision.

User-facing output is scan-first rather than line-count-first. The first visible block passes when a busy reader can locate the result, why it matters, current flow/source, decisive evidence or uncertainty, and next action without parsing internal history. Put commands, complete inventories, research trails, and secondary risks afterward or behind exact pointers. Never stack a result header, Change Brief, expert note, walkthrough, and handoff when they repeat the same fact; compose one view and omit duplicate fields. No arbitrary reading-time, terminal-screen, line, or anchor count is a correctness rule.
...
When two or three genuinely viable user-owned outcomes remain, show the recommendation and every viable alternative together in the first decision view. When four or more remain, preserve them all but first ask one bounded discriminator with at most three exhaustive groups; after that answer, show every viable outcome in the selected group, repeating only if needed. Never silently remove an outcome merely to satisfy the three-choice cap, and exclude one only with evidence that it is not currently viable. Each option names one semantic action, its observable result, and its actual tradeoff; never offer an unsafe/unverified intermediate state as selectable, identify choices only by numbers or letters, or bundle a required closure action with optional continuation. Required deterministic bookkeeping, metadata repinning, and role-owned repair are executed and reported rather than presented as user choices. Put audit-scale path lists, test inventories, and internal IDs behind an artifact or scoped inspection command so the decision remains directly scannable.
...
At a natural Feature boundary, the last Task PASS is necessary evidence but never sufficient proof that the whole Feature intent landed. Architect performs the bounded convergence defined by its role against current approved scope/slices, accepted candidates, live source, and decisive evidence; it materializes a still-specified open slice without ceremony, routes a real intent/structure conflict to its existing owner, and distinguishes complete from user-approved deferred work. A historical Architecture without a slice map is reconstructed only from already-approved evidence and never treated as completion evidence when coverage remains ambiguous. This reuses Architecture, Task, Review, source, and state rather than creating another artifact or universal Gate.
...
After the core problem, recommendation/accepted result, and next action are understandable, integrate a bounded expert note only when a non-obvious principle would help the user maintain the change, recognize a recurring failure, or avoid a material mistake. Prefer one useful idea; add more only when the current high-risk decision truly needs them. Render `plain meaning -> precise term -> current code/evidence anchor -> reusable rule`, omit mechanical, repeated, speculative, or unrelated knowledge, and offer deeper theory on request. This is a presentation/default behavior, never a new artifact, session, quiz, PASS condition, or user Gate. If the user is confused, simplify the core explanation before adding more expertise.
...
- Study external material when a material decision depends on an unstable fact, unfamiliar API or domain concept, normative standard, security/safety rule, costly-to-reverse architecture, explicit reference/original behavior, or a knowledge gap that project evidence cannot answer. Skip it for mechanical, low-consequence, or already-determined work. Prefer official/primary sources and verified reference behavior; record role, scope, date/revision, supported claim, project fit/difference, adopted principle, and re-check trigger while keeping inference explicit.
- A user-approved target/original/compatibility reference is part of intent for only its approved behavior scope; it is not merely an implementation example and does not silently govern unrelated behavior. A reference implementation, analogous case, domain principle, and normative standard remain distinct evidence roles.
- The AI studies and synthesizes before asking the user to study. User-facing output gives the applicable principle, project fit/difference, what was deliberately not copied, current Architecture/source anchor, and reconsider condition only when those facts materially shape the result; research history stays behind evidence.

...
→ build candidate + progressive source orientation
→ independent review
→ accepted change + risk-scaled Change Brief + verified compact Diff/source walkthrough
...
- Build and Review: once exact implementation entry points are known, Builder exposes a small non-blocking source orientation and updates it only when responsibility or runtime flow materially changes. Review PASS verifies that cumulative map against the exact candidate, shows only the three-to-five primary reading anchors plus the full-map pointer, and uses only the configured durable inspection pause. Use conceptual/runtime order before file order; mechanical/non-code changes never gain a ceremonial pause.

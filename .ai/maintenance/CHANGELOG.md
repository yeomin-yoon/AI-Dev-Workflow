...
- Added a Diff-first `CODE_WALKTHROUGH` that binds every non-trivial reviewed source change to its exact snapshot, gives each changed hand-written production file a plain role, and guides direct per-file source reading in runtime order.

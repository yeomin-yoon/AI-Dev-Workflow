...
Before staging, append the single accepted-Task line defined in `.ai/contracts/STATE.md#run-ledger` with `closure: main_checkpoint`, and add that one path to `include` as role-owned Task-closure evidence. It is a silent projection of artifacts that already exist: it never appears in `COMMIT_DONE`, never becomes a choice, blocker, or Review finding, and a failed append never blocks the route or the next Task.

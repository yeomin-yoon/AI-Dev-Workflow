...
| `reviewing/active` | a Build/Review repair carrying `resume_integration` PASSes, and Main Front Desk records the repaired committed `main_after` plus a new full `original main_before..repaired main_after` range | `integration/active`; clear blocker fields and never reuse the old Integration verdict | independent main Reviewer with a new Integration Review attempt |

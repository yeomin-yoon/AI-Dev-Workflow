...
4. Record cause, evidence, changed artifacts, verification, residual risk, next role/action, and minimum input paths only in an artifact the active role may write and in state. Never edit another role's artifact.
5. Return exactly one outcome: `resolved`, `routed`, or `blocked`. Do not restart the whole workflow or cross role boundaries.
...
If `owner=user`, immediately follow the outcome with the `ACTION_CARDS.md` User Action Card. State why tools cannot obtain the evidence, give exact app/path/steps and observable PASS/FAIL, provide a copyable reply, and name the safe fallback/consequence. A route such as `provide Editor/PIE evidence` is incomplete by itself.
...
For unavailable manual verification after implementation is complete, route the Build candidate to Reviewer first. Reviewer evaluates all available evidence and owns the user observation gate; do not repeatedly send the user back to Builder just to explain how to perform the check.
...
Every non-`main` close emits the `RETURN_TO_MAIN` card from `.ai/contracts/MAIN_DESK.md`. The user pastes its single instruction into main Work Front Desk, which verifies the referenced files/Git and issues any replacement `NEXT_SESSION`. Do not make the user retain or edit old Lane prompts. A main Work Front Desk replacement is the only exception: create it directly with the fixed initial compact main Work Bootstrap prompt.

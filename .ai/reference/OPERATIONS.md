...
4. Build one bounded evidence batch before requesting user help. Use source and deterministic tools first, and reuse applicable verified precedent/evidence through `ARCHITECT.md#decision-evidence-ladder` before inventing another probe. When an editor, device, binary asset, or experiential behavior is unavailable, ask for the smallest user action that distinguishes the remaining hypotheses and batch observations available in the same app/surface. Do not drip-feed avoidable one-field questions or repeat evidence already supplied.
...
Only `current_blocker` enters the Exception procedure below. Never downgrade a blocker for momentum; if the available evidence cannot safely distinguish blocker from follow-up, run one bounded probe or use the existing `context`/`verification` route. A probe is the last evidence step, not a default implementation strategy: state its one unresolved question, discriminating observations, minimum disposable scope, and rollback/stop condition, and do not run it when approved intent, applicable verified precedent, source, or official documentation already answers the question.

...
1. `gate necessity`: refusal or another viable answer must materially change user-owned behavior, scope, public compatibility, irreversible/external effects, cost/risk, or mandatory human acceptance. If refusal would not change the next action, only one safe path is viable, or the difference is a local/reversible mechanism with the same approved observable result, continue without a Gate.
2. `decision readiness`: the chat brief must make the problem, observable option differences, recommendation/default, defer/reject consequence, evidence, and uncertainty understandable in `user_language` without opening the internal English Architecture file. If targeted evidence can resolve the uncertainty, investigate first. If the user cannot reasonably choose yet, propose the smallest discriminating probe or a provisional reversible default when safe; block only for unresolved user-owned intent or material risk.

An affirmative reply to a brief that fails either check is not durable approval. Include the affected existing behavior, what moves where, the important tradeoff, what remains unchanged/out of scope, and the first implementation slice. The artifact link is supporting evidence, not the explanation.

Whenever saying something remains unchanged, name the exact prior behavior/contract and its observable invariants. A phrase such as `request behavior stays the same` is insufficient unless the brief names the compared revision or contract and the evidenced inputs, outputs, state transitions, side effects, and error behavior that remain true. Use only invariants actually supported by project evidence.

## Approval policy

Require user approval for an Architecture Gate and for a Task only when the Gate passes both necessity and readiness above and adds new user-owned intent, materially different observable behavior or public compatibility, irreversible/external effects, material cost or scope growth, or a mandatory human acceptance gate. Once an Architecture and its boundaries are approved, routine Tasks that merely realize it may be approved by Architect and handed to Builder without another user interruption. Record why the Task is covered by the prior approval.

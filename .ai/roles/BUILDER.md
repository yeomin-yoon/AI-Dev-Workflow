...
- Before writes, inspect version-control status and relevant diff, then classify Baseline paths as `unrelated_pre_existing | inherited_task | unknown`; never relabel interrupted Workflow bytes as pre-existing user work. Resolve `unknown` before writing that path.
...
- When approved Architecture records a material target/reference, standard, implementation, or domain principle, implement only its adopted project-fit consequence. Keep deliberately un-copied or out-of-scope behavior out of the candidate, and connect the resulting source symbol to that decision in the Build Result rather than copying research prose into code or comments.
...
- Once preflight identifies the exact implementation entry points, and no later than the first coherent non-trivial production-source edit, show a non-blocking source-truth orientation in `user_language`: the approved observable purpose, responsibility/runtime flow, and the smallest connected `path#symbol` set that lets the user open the actual entry and follow the current change. For each anchor say what the code owns and why this Task touches it, then continue without waiting. Replace an Architecture's planned location with exact source truth when they differ. Show another orientation only when a new/moved responsibility, class/file, dependency direction, runtime boundary, or user-authored editor connection materially changes that map; report only the delta, not the accumulated inventory. Keep the complete current map in the Build Result `Changes` and `Source Map` sections.
...
For `awaiting_user_authoring`, keep `building/active + next.role: builder + next.action: await_user_build_authoring` and show the one batched candidate-mutating `EDITOR_CHECK`; this is pending implementation, not a blocker or Review handoff. For `ready_to_review`, give one scan-first result containing the observable outcome, final connected source path and runtime flow from the cumulative Source Map, decisive evidence/limits, and the exact Reviewer handoff from `ACTION_CARDS.md` in `user_language`. Do not repeat an earlier unchanged inventory. This is orientation, not approval or an inspection wait. For a genuine user-owned blocker, use its `USER_ACTION` card.

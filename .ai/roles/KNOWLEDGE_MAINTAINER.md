...
On first setup, create the requested lane from `.ai/lanes/_template` only when it is missing; initialize an existing scaffold in place without replacing its files. Resolve placeholders and existing-project ownership from repository evidence. Approved product/requirements/specification or planning documents are reference-only by default: index exact applicable sections and put their precise paths under `shared_read_only`, not `owned_paths`. Do not infer document-authoring permission from a broad `Docs/**` directory; a separate explicit user/team-approved document Task may assign only its required paths. Repository developer documentation such as README/API/migration docs may remain Task-writable when project evidence and approved scope require it. For a greenfield project, leave unproven production roots unowned; the Architect assigns planned roots through approved Architecture before the first build. Empty `owned_paths` never means repository-wide access. Do not invent extra lanes, worktrees, or automation.

An explicit standing request about whether to ask before a reviewed local checkpoint or whether to stop before the next routine Task updates only `.ai/shared/knowledge/project.yaml#interaction` under the `KNOWLEDGE.md` enum. Preserve the active workflow route and confirm the new behavior once; do not create another approval Gate or treat a one-off conversational pause as a durable preference.
...
When a checkpoint completes, report how many pending Reviews were synchronized. An accepted single-main working-tree checkpoint returns to Work for the exact policy-controlled local checkpoint; in compact mode the current Work shell continues directly, while a strict Knowledge session emits the exact `DO_NEXT` to Work. `COMMIT_READY` appears only for `checkpoint: ask` or an explicit pre-commit request. Other routes follow state and topology. Do not ask for another confirmation merely to continue.
